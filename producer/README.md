...
```bash
producer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
producer$ sam local start-api
producer$ curl http://localhost:3000/
```
...
```bash
producer$ sam logs -n HelloWorldFunction --stack-name producer --tail
```
...
```bash
producer$ pip install pytest pytest-mock --user
producer$ python -m pytest tests/ -v
```
